...
cp "${PROJ_DIR}/plugin/ClockPopup.qml" "${DEST}/ClockPopup.qml"
...
echo "NOTE: QML/C++ edits are only picked up after a shell restart or re-login"
echo "  (the plugin watcher does not re-parse entry QML). Use"
echo "  scripts/reload-shell.sh to restart without ever leaving a duplicate bar:"